...
def ProcessMenuOption(option):
    ...
    if (command == "download"):
        ...
        CommandDownload_ml4t(filename)
        quit()

    if (command == "download_group"):
        CommandDownload_ml4t_group(filename)
        quit()
...
def PromptMenu():
    ...
    print("> mentor")
    print("> downnload_group folder_to_put_files_in")
...
def CommandDownload_ml4t_group(foldername):
    global canvasCourseUsers
    global course
    global assignment
    ...
    for user in courseGroupUsers:
        ...
        if (submission != None):
            if ("attachments" in submission):
                ...
                # add a subfolder for each student to put files into - if have attachment (submission)
                student_name = user["sortable_name"]
                student_name = "".join([c for c in student_name if c.isalpha() or c.isdigit() or c == ' ']).rstrip()

                filename = foldername + student_name
                if (os.path.exists(filename) == False):
                    print("Creating student folder: " + filename)
                ...
                for attachment in attachments:
                    ...
                    if (link == ""):
                        continue
                    print("Downloading: " + link + " [to] " + filename)
                    DownloadURLToFile_ml4t(link, filename)

    print("Done!")

def CommandDownload_ml4t(foldername):
    global canvasCourseUsers
    global course
    global assignment
...
def DownloadURLToFile_ml4t(url, folder_name):
    response = requests.get(url)
    ...
    #strip if find dash
    if file_name:
        i_dash = file_name.find("-")
        if i_dash > -1:
            i_dot = file_name.find(".")
            file_name = file_name[0:i_dash]+file_name[i_dot:len(file_name)]
